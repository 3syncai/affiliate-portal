...
export default function StateAdminLayout({
    ...
    useEffect(() => {
        ...
        try {
            ...
            // Fetch notifications
            if (parsed.id) {
                fetch(`/api/notifications?recipientId=${parsed.id}&recipientRole=state`)
                    .then(res => res.json())
                    .then(data => {
                        if (data.success) {
    ...
    return (
        <div className="min-h-screen flex overflow-x-hidden" style={{ backgroundColor: theme.background }}>
                ...
                <div className="p-4 relative" style={{ borderTopColor: 'rgba(255,255,255,0.2)', borderTopWidth: '1px' }}>
                    <div className="flex items-center gap-3 px-4 py-2 mb-2">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-white/70 truncate">{user?.state}</p>
                        </div>
                        <button
            ...
            {/* Main Content */}
            <div className={`flex-1 flex flex-col ${sidebarOpen ? "ml-72" : "ml-0"} transition-all duration-300 overflow-x-hidden`}>
                {/* Top Bar */}
                <header className="bg-white/80 backdrop-blur-xl border-b border-indigo-50 px-8 py-5 sticky top-0 z-20 flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-6">
                        {/* Notification Bell */}
                        {user?.id && (
                            <NotificationDropdown userId={user.id} userRole="state" />
                        )}
