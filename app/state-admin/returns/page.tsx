...
export default function StateAdminReturnsPage() {
  ...
  useEffect(() => {
    ...
    setUser(parsed);
    if (parsed.state) {
      fetchReturns(parsed.state);
    }
  }, []);

  const fetchReturns = async (state: string) => {
    setLoading(true);
    try {
      const response = await axios.get(
        `/api/state-admin/returns?state=${encodeURIComponent(state)}`,
      );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Cancelled and return-requested orders in {user?.state}
        </p>
      </div>
