...
import type { ReferralCategory } from "@/lib/returns-response";

type IssueType = "cancelled" | "return_requested" | "both";
...
export default function StateAdminReturnsPage() {
  ...
  const [user, setUser] = useState<StateAdminUser | null>(null);

  useEffect(() => {
    const userData = localStorage.getItem("affiliate_user");
    if (!userData) return;
    const parsed = JSON.parse(userData) as StateAdminUser;
    setUser(parsed);
    if (parsed.state) {
      fetchReturns(parsed);
    }
  ...
  const fetchReturns = async (adminUser: StateAdminUser) => {
    setLoading(true);
    try {
      ...
      const response = await axios.get(
        `/api/state-admin/returns?${params.toString()}`,
      );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Self-referral and team sales returns in {user?.state}
        </p>
      </div>
