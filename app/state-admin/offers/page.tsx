...
export default function StateAdminOffersPage() {
  const [user, setUser] = useState<any>(null)
  const [products, setProducts] = useState<Product[]>([])
  const [loadingProducts, setLoadingProducts] = useState(true)
  const [directRate, setDirectRate] = useState<number>(0)
  const { data, isLoading } = useSWR("/api/additional-commissions/active?role=state", fetcher)
  ...
  const fetchRates = async () => {
    try {
      ...
      if (response.data?.success && response.data?.summary?.state) {
        setDirectRate(Number(response.data.summary.state.directRate) || 0)
      }
    } catch (error) {
    ...
    <div className="group relative bg-white rounded-2xl border-2 border-gray-100 overflow-hidden hover:border-emerald-200 transition-all duration-300 hover:shadow-xl">
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
          ...
          <div className="text-sm font-semibold mt-1 text-emerald-700">
            +{additionalRate}% (₹
