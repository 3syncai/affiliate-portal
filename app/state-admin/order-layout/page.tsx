...
export default function OrderLayoutPage() {
  ...
  const getStatusBadge = (status: string) => {
    const statusColors: Record<string, string> = {
      ...
      "CREDITED": "bg-emerald-50 text-emerald-700 border-emerald-200 ring-emerald-100"
    }
  ...
  return (
    <div className="space-y-6 sm:space-y-8 bg-gray-50/50 -m-4 p-4 sm:-m-6 sm:p-6 min-w-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <Link href="/state-admin/returns" className="block min-w-0">
              ...
              {showStatusMenu && (
                <>
                  ...
                  <div className="absolute left-0 top-full mt-2 w-full rounded-xl border border-gray-200 bg-white shadow-lg overflow-hidden z-20">
                    {["ALL", "PENDING", "APPROVED", "PAID", "REJECTED", "CREDITED"].map((value) => (
                      <button
                        key={value}
                        type="button"
                        onClick={() => {
              ...
              {filteredOrders.map((order) => (
                <div key={order.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 min-w-0">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3 text-xs">
                    ...
                    <div>
                      <p className="text-[10px] uppercase text-gray-400 font-semibold">Sales Executive</p>
                      <p className="font-semibold text-gray-700">{order.affiliate_earned > 0 ? formatCurrency(order.affiliate_earned) : "-"}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase text-gray-400 font-semibold">Branch</p>
                      <p className="font-semibold text-gray-700">{order.branch_earned > 0 ? formatCurrency(order.branch_earned) : "-"}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase text-gray-400 font-semibold">ASM</p>
                      <p className="font-semibold text-gray-700">{order.asm_earned > 0 ? formatCurrency(order.asm_earned) : "-"}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase text-gray-400 font-semibold">My Earned</p>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr className="bg-gray-50">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Sale By</th>
                  ...
                  {filteredOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-4 py-4 whitespace-nowrap">
                        <div className="text-sm font-mono font-medium" style={{ color: theme.primary }}>{order.order_id}</div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-600">{formatDate(order.created_at)}</td>
                      ...
                      <td className="px-4 py-4">
                        <div className="text-sm text-gray-900 max-w-xs truncate" title={order.product_name}>{order.product_name}</div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-right text-gray-900 font-semibold tabular-nums">{order.quantity}</td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-right font-bold text-blue-600 tabular-nums">{formatCurrency(order.order_amount)}</td>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4">
          ...
          />
          <div className="relative bg-white rounded-t-2xl md:rounded-2xl max-w-4xl w-full max-h-[90dvh] overflow-y-auto shadow-2xl">
