...
export default function ASMReturnsPage() {
  ...
  const [user, setUser] = useState<{ city?: string; state?: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (parsed.city && parsed.state) {
      fetchReturns(parsed.city, parsed.state);
    }
  }, []);

  const fetchReturns = async (city: string, state: string) => {
    setLoading(true);
    try {
      const response = await axios.get(
        `/api/asm/returns?city=${encodeURIComponent(city)}&state=${encodeURIComponent(state)}`,
      );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Cancelled and return-requested orders in {territoryLabel}
        </p>
      </div>
