...
export default function StateAdminsPage() {
    ...
    const fetchStateAdmins = async () => {
        setLoading(true)
        try {
            const response = await axios.get("/api/admin/state-admins")
    ...
    const toggleStatus = async (adminId: string, currentStatus: boolean) => {
        try {
            await axios.post("/api/admin/state-admins/toggle-status", {
                adminId,
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-lg text-gray-500">Loading state admins...</div>
            </div>
        )
    }
