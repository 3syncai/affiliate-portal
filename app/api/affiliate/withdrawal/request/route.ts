...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Re-query the real balance server-side — never trust the frontend value
    const affiliateRateRes = await pool.query(
    ...
    const values = [
      ...
      affiliate.email,
      withdrawalAmount,
      tdsPercentage || gstPercentage || 0,
      tdsAmount || gstAmount || 0,
      netPayable,
      paymentMethod,
