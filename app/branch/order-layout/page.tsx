...
export default function OrderLayoutPage() {
  ...
  useEffect(() => {
    ...
    if (userRole) {
      // Map frontend roles to backend commission_source roles
      if (userRole === "asm") {
        setRole("area_manager") // DB label for Area Manager
      } else if (userRole === "branch") {
        setRole("branch_admin") // DB label for Branch Admin/ASM
      } else {
        setRole(userRole)
      }
    }
  ...
  const { data, isLoading } = useSWR(
    user?.id && role ? `/api/branch/orders?adminId=${user.id}&role=${role}` : null,
    fetcher
  )
